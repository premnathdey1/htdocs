...
var colors=['#01888C','#FC7500','#034F5D','#F73F01','#FC1960','#C7144C','#F3C100','#1598F2','#2465E1','#F19E02',]
var MersenneTwister=require('mersenne-twister');var SVG_NS='http://www.w3.org/2000/svg'
function createNode(type){return document.createElementNS(SVG_NS,type)}
function setAttribute(node,attribute,value){node.setAttributeNS(null,attribute,value)}
module.exports=function createLogo(options_){var options=options_||{}
...
var twister
if(colorSeed){twister=new MersenneTwister(colorSeed)}
...
var toDraw=[]
if(!options.pxNotRatio){width=(window.innerWidth*(options.width||0.25))|0
height=((window.innerHeight*options.height)||width)|0
if('minWidth'in options&&width<options.minWidth){width=options.minWidth
height=(options.minWidth*options.height/options.width)|0}}
setAttribute(container,'width',width+'px')
setAttribute(container,'height',height+'px')
function setLookAt(target){var bounds=container.getBoundingClientRect()
mouse.x=1.0-2.0*(target.x-bounds.left)/bounds.width
mouse.y=1.0-2.0*(target.y-bounds.top)/bounds.height}
document.body.appendChild(container);(function(){var pp=foxJSON.positions
var ptr=0
for(var i=0;i<pp.length;++i){var p=pp[i]
for(var j=0;j<3;++j){positions[ptr++]=p[j]}}})()
function Polygon(svg,indices,chunk){this.svg=svg
this.indices=indices
this.zIndex=0
this.chunk=chunk}
var polygons=(function(){var polygons=[]
for(var i=0;i<foxJSON.chunks.length;++i){var chunk=foxJSON.chunks[i]
if(!chunk.polygons){chunk.polygons=[]}
var color
if(twister){color=colors[Math.floor(twister.random()*colors.length)]}else{color='rgb('+chunk.color+')'}
var faces=chunk.faces
for(var j=0;j<faces.length;++j){var f=faces[j]
...
return polygons})()
var computeMatrix=(function(){var objectCenter=new Float32Array(3)
...
return function(){var rect=container.getBoundingClientRect()
...
if(slowDrift){var time=(Date.now()/1000.0)
...
return transformed}})()
function updatePositions(M){var m00=M[0]
var m01=M[1]
var m02=M[2]
var m03=M[3]
var m10=M[4]
...
var m32=M[14]
var m33=M[15]
for(var i=0;i<NUM_VERTS;++i){var x=positions[3*i]
...
function compareZ(a,b){return b.zIndex-a.zIndex}
function updateFaces(){var i
var rect=container.getBoundingClientRect()
var w=rect.width
var h=rect.height
toDraw.length=0
for(i=0;i<polygons.length;++i){var poly=polygons[i]
...
for(var j=0;j<3;++j){var idx=indices[j]
points.push(0.5*w*(1.0-transformed[3*idx])+','+
...
if(joinedPoints.indexOf('NaN')===-1){setAttribute(element,'points',joinedPoints)}
toDraw.push(poly)}
toDraw.sort(compareZ)
container.innerHTML=''
for(i=0;i<toDraw.length;++i){container.appendChild(toDraw[i].svg)}}
...
window.addEventListener('mousemove',function(ev){if(!shouldRender){startAnimation()}
if(followCursor){setLookAt({x:ev.clientX,y:ev.clientY})
renderScene()}})
window.addEventListener('deviceorientation',function(event){if(!shouldRender){startAnimation()}
if(followMotion){const leftToRight=event.gamma
const frontToBack=event.beta
const xOffset=200
const yOffset=-300
const acceleration=10
setLookAt({x:xOffset+leftToRight*acceleration,y:yOffset+frontToBack*acceleration})
renderScene()}})
function renderScene(){if(!shouldRender)return
...
return out;};},{}],10:[function(require,module,exports){module.exports=transformMat4;function transformMat4(out,a,m){var x=a[0],y=a[1],z=a[2],w=m[3]*x+m[7]*y+m[11]*z+m[15]
w=w||1.0
out[0]=(m[0]*x+m[4]*y+m[8]*z+m[12])/w
out[1]=(m[1]*x+m[5]*y+m[9]*z+m[13])/w
out[2]=(m[2]*x+m[6]*y+m[10]*z+m[14])/w
return out}},{}],11:[function(require,module,exports){var MersenneTwister=function(seed){if(seed==undefined){seed=new Date().getTime();}
...
module.exports=MersenneTwister;},{}]},{},[1]);